...
# Cron-friendly wrapper to process STFC CR1000X PTB110 pressure data for one UTC date.
# Usage:
#   ./process_stfc_date.sh YYYYMMDD
...
if ! date -u -d "${DATE_UTC}" +%Y%m%d >/dev/null 2>&1; then
    echo "Error: invalid date '${DATE_UTC}'" >&2
    exit 1
fi
...
RSYNC_DEST=${RSYNC_DEST:-${GWS_ROOT}/raw_data/cao-surface-met/data/long-term/loggernet/}
SPLIT_INPUT_BASE=${SPLIT_INPUT_BASE:-${RSYNC_DEST}}
RAW_DATA_BASE=${RAW_DATA_BASE:-${GWS_ROOT}/raw_data/cao-surface-met/data/long-term/new_daily_split}
...
LOG_FILE="${LOG_DIR}/ptb110_stfc_single_date_${DATE_UTC}_${STAMP}.log"
...
if command -v flock >/dev/null 2>&1; then
    LOCK_FILE="/tmp/ptb110_stfc_single_date.lock"
    exec 9>"${LOCK_FILE}"
    if ! flock -n 9; then
...

{
    ...
    YEAR=$(date -u -d "${DATE_UTC}" +%Y)
    YEAR_MONTH=$(date -u -d "${DATE_UTC}" +%Y%m)
